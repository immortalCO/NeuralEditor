...
print(f"Train field_dtu_{SCENE}", flush=True)
...
fh = logging.FileHandler(f"scratch/train_field_dtu_{SCENE}.log")
...
DATASET = f"./datasets/Scratch/dtu_{SCENE}"
CLOUD_NAME = "cloud_pn"
...
if SCENE not in ['scan1', 'scan114']:
    # manually fix the scale
    pts *= 200
...
plot(pts[mask], conf=pts_conf[mask], rgb=pts_col[mask], 
     save=f"./scratch/dtu_{SCENE}_{CLOUD_NAME}_cloud.png")
...
def pix2pts_dtu(cam, pix_ind, scale=1, rand=False):
    cam_mat_inv = cam[1]
    n, m = IMG_H * scale, IMG_W * scale
    tmp = torch.stack([pix_ind % m, pix_ind.div(m, rounding_mode='floor')], dim=-1).double()
    tmp = tmp + torch.tensor([80, 44]) * scale
    tmp = (tmp * 2 + 1).double() 
    if rand:
...
def pts2pix_dtu(cam, pts):
    pts = pts / CLOUD_SCALE
    cam_mat = cam[0]
    proj = dehomo(cam_mat.matmul(enhomo(pts).double().unsqueeze(-1)).squeeze(-1)).float()
    proj = torch.stack([proj[:, 1], proj[:, 0]], dim=-1)
    proj = (proj / 2) - torch.tensor([44, 80])
    return proj.float()

def pix2ray_dtu(cam, pix_ind, scale=1, rand=False):
    center = cam[2]
    pts = pix2pts_dtu(cam, pix_ind, scale=scale, rand=rand)
    dir = (pts - center)
    dir /= dir.norm(dim=-1, keepdim=True).clamp(min=1e-7)
    return dir.float()

def read_data_dtu(config, i, for_eval=False, debug=False, ray_upsample=1, full_image=False, return_fn=True):
    import cv2
    import math
    
    img_file = f"{DATASET}/{config['frames'][i]['file_path']}"
    cam_int = torch.tensor(config['frames'][i]['cam_int'], dtype=torch.double)
    cam_ext = torch.tensor(config['frames'][i]['cam_ext'], dtype=torch.double)
    ...
    if debug:
        ...
        restored = pts2pix_dtu(cam, pix2pts_dtu(cam, pixels)).long()
    ...
    if for_eval or (not return_fn):
        ...
        line_d_fn = lambda : pix2ray_dtu(cam, pix_used, scale=ray_upsample, rand=TRAIN_RANDOM_AUG and not for_eval)
        line_d = line_d_fn()
        ...
        def generate_data():
            ...
            pix = pix_all.gather(1, cho).squeeze(1)
            ans = ans_all.gather(1, cho.unsqueeze(-1).expand(-1, -1, 4)).squeeze(1)
            line_d = pix2ray_dtu(cam, pix, scale=ray_upsample, rand=TRAIN_RANDOM_AUG)
            return line_d, ans
...
line_o, data_fn, img_outlier_fn = read_data_dtu(valid_config, 0, debug=True, full_image=False, ray_upsample=3)
...
if train_config is not None and not eval_only:
    ...
    for i in tqdm(range(len(train_config['frames']))):
        import gc
        gc.collect()
        o, fn, ofn = read_data_dtu(train_config, i, ray_upsample=IMAGE_SCALE_TRAIN, return_fn=True)
        train_o.append(o)
        train_fn.append(fn)
...
if train_config is not None and not eval_only:
    if grow_extra_config is not None:
        for i in tqdm(range(len(grow_extra_config['frames']))):
            o, fn = read_data_dtu(grow_extra_config, i, ray_upsample=IMAGE_SCALE_TRAIN, return_fn=True)
            grow_o.append(o)
            grow_fn.append(fn)
...
if train_config is not None and not eval_only:
    if prune_extra_config is not None:
        for i in tqdm(range(len(prune_extra_config['frames']))):
            o, fn, ofn = read_data_dtu(prune_extra_config, i, ray_upsample=1, return_fn=True)
            prune_outlier_fn.append(ofn)
...
for i in tqdm(range(len(valid_config['frames']))):
    o, d, a, eval_info = read_data_dtu(valid_config, i, for_eval=True, ray_upsample=IMAGE_SCALE)
...
if train_config is not None and not eval_only:
    ...
    plot(pts, conf=pts_conf, rgb=pts_col.clamp(min=0,max=1), 
        save=f"./scratch/dtu_{SCENE}_{CLOUD_NAME}_cloud_imgoutlier.png")
...
def create_field(pts, pts_embed, pts_conf):
    return KDField(embed_dim=32, #32, 
                   ...
                   model_background_reg_weight=1000,
...
plot(field.pts.cpu()[mask], rgb=norm_est[mask]*0.5+0.5, 
     save=f"./scratch/dtu_{SCENE}_{CLOUD_NAME}_norm_mesh.png")
...
def evaluate(field, valid_dataset, show_sample=False, show_all=False, show_predict_only=False,
    ...
    if norm_map:
        ...
        plot(pts[mask], rgb=norms[mask]*0.5+0.5, dpi=64,
             save=f"./scratch/dtu_{SCENE}_{CLOUD_NAME}_norm_pred.png")
    ...
    for i, (img, eval_info) in enumerate(tbar):
        with logging_redirect_tqdm():
            ...
            if show_all:
                ...
                show(a.numpy(),
                    save=f"./scratch/dtu_{SCENE}_{CLOUD_NAME}_valid{i}_pred.png")
                
                if not show_predict_only:
                    logging.debug(f"Target:")
                    show(b.numpy(),
                        save=f"./scratch/dtu_{SCENE}_{CLOUD_NAME}_valid{i}_target.png")
                    logging.debug(f"Diff:")
                    show((a - b).abs().mean(dim=-1).clamp(min=0, max=1).numpy(),
                        save=f"./scratch/dtu_{SCENE}_{CLOUD_NAME}_valid{i}_diff.png") 
    ...
    if (not show_all) and show_sample is not None:
        ...
        logging.debug(f"Predict:")
        show(a.numpy(),
             save=f"./scratch/dtu_{SCENE}_{CLOUD_NAME}_valid{show_sample}_pred.png")
        if not show_predict_only:
            logging.debug(f"Target:")
            show(b.numpy(),
                save=f"./scratch/dtu_{SCENE}_{CLOUD_NAME}_valid{show_sample}_target.png")
            logging.debug(f"Diff:")
            show((a - b).abs().mean(dim=-1).clamp(min=0, max=1).numpy(),
                save=f"./scratch/dtu_{SCENE}_{CLOUD_NAME}_valid{show_sample}_diff.png") 
...
def train(field, exp_name="field", OUTPUT="./scratch/", lr=5e-3, min_lr=1e-6, valid_epoch=1, eloss_coef=0,
    ...
    def debug_plot(name, pts, rgb):
        ...
        mask = torch.randperm(pts.shape[0])[:524288]
        plot(pts[mask], rgb=rgb[mask], save=f"./scratch/dtu_{SCENE}_{CLOUD_NAME}_debug_{name}.png")
...
if LOAD_CKPT is not None:
    ckpt_file = f"./scratch/field_dtu_{SCENE}_{CLOUD_NAME}_layer{MAX_LAYER}_{LABEL}_train_{LOAD_CKPT}.ckpt"
...
if not eval_only:
    train(field, f"field_dtu_{SCENE}_{CLOUD_NAME}_layer{MAX_LAYER}_{LABEL}", 
        lr=2e-3, eloss_coef=1e-5,
        conf_reg_coef=0.0002,
        valid_epoch=1,
        num_epoch=50,
        sch_T=40, grow_sch_T_fact=1, grow_epoch_fact=1/40, rebuild_tree_fact=4,
        start_epoch=start_epoch,
        resume_ckpt=resume_ckpt,
        first_grow_epoch=3, last_grow_epoch=12, 
